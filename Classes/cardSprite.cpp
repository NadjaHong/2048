...
{
	number = num;
	cardBgColour = LayerColor::create(Color4B(200, 190, 180, 255), cardWidth*0.8975, cardHeight*0.8975);
	cardBgColour->setPosition(Point(px, py));
	if (number > 0) //如果有数字，则显示数字
	{
		cardLabel = LabelTTF::create(String::createWithFormat("%d", number)->getCString(), "Arial", 80);
	...
	{
		cardLabel = LabelTTF::create("", "Arial",80);
		cardLabel->setPosition(cardBgColour->getContentSize().width / 2, cardBgColour->getContentSize().height / 2);
...
void CardSprite::setNumber(int num)
{
	...
	{
		cardLabel->setFontSize(80);
	}
	if (number >= 16 && number < 128)
	{
		cardLabel->setFontSize(70);
	}
	if (number >= 128 && number < 1024)
	{
		cardLabel->setFontSize(60);
	}
	if (number >= 1024)
	{
	...
	if (number == 2)
	{
		cardBgColour->setColor(Color3B(230, 200, 150));
	}
	if (number == 4)
	{
