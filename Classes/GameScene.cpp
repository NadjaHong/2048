...
bool GameScene::init()
{
	...
	//添加标题
	auto title = Label::create("2048", "Arial", 80);
	title->setColor(Color3B(255, 255, 153));
	title->setPosition(Point(visibleSize.width *0.23, visibleSize.height/1.2));
	this->addChild(title);
	//加入restart按钮  
	restartBtn = Sprite::create("replay.png");
	restartBtn->setPosition(Point(visibleSize.width *0.555, visibleSize.height/1.2));
	restartBtn->setScale(0.9f);
	this->addChild(restartBtn);
	//添加声音切换按钮
	//初始化获取最好分数和声音变量，第一次启动应用的话xml里没有任何值，所以下面的会返回0和false
	sound = UserDefault::getInstance()->getBoolForKey("SOUND");

	MenuItemImage *sound_turnOn,*sound_turnOff;
	sound_turnOn = MenuItemImage::create("sound-on.png",
										 "sound-on.png");
	sound_turnOff = MenuItemImage::create("sound-off.png",
									   	"sound-off.png");
    MenuItemToggle *toggleItem = MenuItemToggle::createWithCallback(CC_CALLBACK_1(GameScene::menuSoundCallback, this),
		sound_turnOn,sound_turnOff, NULL);
	
	toggleItem->setScale(0.9f);
	toggleItem->setPosition(Point(visibleSize.width*0.825,visibleSize.height/1.2));
	auto menu = Menu::create(toggleItem, NULL);
    menu->setPosition(Point::ZERO);
    this->addChild(menu);

	//加入游戏分数  
	auto slabel = Label::create("Score", "Arial", 60);
	slabel->setPosition(Point(visibleSize.width / 5, visibleSize.height/1.5));
	this->addChild(slabel);
    score = 0;
    scoreLabel = Label::create("0", "Arial", 60);
	scoreLabel->setColor(Color3B(70, 255, 203));
	scoreLabel->setPosition(Point(visibleSize.width / 2+30, visibleSize.height/1.5));
...
bool GameScene::onTouchBegan(Touch *touch, Event *event)
{
	...
	if (restartBtn->getBoundingBox().containsPoint(touch->getLocation()))
		Director::getInstance()->replaceScene(TransitionFade::create(0.7f, GameScene::createScene()));
	return true;
}
...
{
	//int space = 15; //卡片间的间隔
	int cardSize = (size.width *0.87) / 4;
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			//最左边留白12，最下面留白size.height/12
			//坐标从左下角算起，右为正，上为正
			CardSprite *card = CardSprite::createCard(0, cardSize, cardSize, cardSize*i + size.width*0.08, cardSize*j +  size.width*0.08 + size.height / 12);
...
bool GameScene::moveLeft()
{
	...
	bool moved = false;
	//计算移动的步进间距
	auto cardSize = (Director::getInstance()->getVisibleSize().width *0.87) / 4;
	//y表示行标号，x表示列标号
			...
			for (int x1 = x + 1; x1 < 4; x1++)
			{
				if (cardArr[x1][y]->getNumber()>0)  //x右边的卡片有数字才动作
				{
					if (cardArr[x][y]->getNumber() == 0)
					{
						...
						auto place = Place::create(Point(cardSize*x1 + Director::getInstance()->getVisibleSize().width*0.08, cardSize*y + Director::getInstance()->getVisibleSize().width*0.08 + Director::getInstance()->getVisibleSize().height / 12));
		...
		for (int y = 0; y < 4; y++)   //内部的N^2复杂度的类似冒泡排序
		{
			for (int y1 = y + 1; y1 < 4; y1++)
			{
				if (cardArr[x][y1]->getNumber()>0)  //x下边的卡片有数字才动作
				{
					if (cardArr[x][y]->getNumber() == 0)
					{
						auto place = Place::create(Point(cardSize*x + Director::getInstance()->getVisibleSize().width*0.08, cardSize*y1 + Director::getInstance()->getVisibleSize().width*0.08 + Director::getInstance()->getVisibleSize().height / 12));
...
void GameScene::checkGameWin()
{
	...
	if (isWin)
	{
		...
		//添加标题
		auto menuTitle = Label::create("YOU WIN", "Arial", 60);
		menuTitle->setPosition(menuSize.width *0.5, menuSize.height *0.60);
		menuLayer->addChild(menuTitle);
		//添加当前分数
		auto menuscoreLabel = Label::create(String::createWithFormat("current: %d", score)->getCString(), "Arial", 40);
		menuscoreLabel->setPosition(menuSize.width *0.5, menuSize.height *0.50);
		...
		auto menuBestscoreLabel = Label::create(String::createWithFormat("best: %d", bestScore)->getCString(), "Arial", 40);
		menuBestscoreLabel->setPosition(menuSize.width *0.5, menuSize.height *0.45);
		...
		menu->setPosition(Point(menuSize.width *0.5, menuSize.height *0.35));
	}
}
...
void GameScene::checkGameOver()
{
	...
	if (isGameOver)
	{
		...
		auto menuSize = menuLayer->getContentSize();
		//添加标题
		auto menuTitle = Label::create("GAME OVER", "Arial", 60);
		menuTitle->setPosition(menuSize.width *0.5, menuSize.height *0.60);
		menuLayer->addChild(menuTitle);
		//添加当前分数
		auto menuscoreLabel = Label::create(String::createWithFormat("current: %d", score)->getCString(), "Arial", 40);
